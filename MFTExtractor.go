...
func main() {
  ...
  file1, err:= os.OpenFile("/Users//armenarsakian//Downloads//MFToutput.txt",os.O_RDWR|os.O_CREATE,0666)
  ...
  for i:=0 ;i<=int(fsize.Size());i+=1024 {
    //if (i<=2048){
       ...
       _, err := file.ReadAt(bs,int64(i))                              
		...
		//	fmt.Printf("MFT------------------------------------------> %d \n",k)	                                                
       //   fmt.Printf(" MFT entry %d seq %d base_ref %d\n",k,r.flags,r.Seq,r.base_ref)// First attribute located at offset %d  and MFT has size %d \n",r.Signature,r.attr_off ,r.flags)
          
	   _,err1:=file1.WriteString("MFT ENTRY------------------------------------------------------------------------"+fmt.Sprintf(" Seq. Num. %d entry %d Descr %s",r.Seq,r.entry ,MFTflags[r.flags])+string(10))
		       if err1 != nil {
			    // handle the error here
			   // fmt.Printf("err %s\n",err)	
			      return
      ...
      if r.Signature != "BAAD" {//enty signature if BAAD error value
		...
		ATR :=  ATRrecordResident{Hexify(Bytereverse(bs[ReadPtr:ReadPtr+4])),readEndian(bs[ReadPtr+4:ReadPtr+8]).(uint32),string(bs[ReadPtr+8:ReadPtr+9]),string(bs[ReadPtr+9:ReadPtr+10]),
		    ...
		    s := strings.Join([] string {"Attr. ",AttrTypes[ATR.Type]}," ")
		   _,err:=file1.WriteString(s+string(10))
		       if err != nil {
			    // handle the error here
			    fmt.Printf("err %s\n",err)	
			      return
		      ...
		      if   ATR.Type== "00000030"       {          // File name
			 ...
			 DecodeUTF16(bs[ReadPtr+ATR.soff+66:ReadPtr+ATR.soff+66+2*uint16(readEndian(bs[ReadPtr+ATR.soff+64:ReadPtr+ATR.soff+65]).(uint8))]),false,false}
			  ...
			  //    fmt.Println(strings.TrimSpace(string(bs[ReadPtr+ATR.soff+66:ReadPtr+ATR.soff+66+2*uint16(readEndian(bs[ReadPtr+ATR.soff+64:ReadPtr+ATR.soff+65]).(uint8))])))
			     s := strings.Join([] string {fmt.Sprintf(" starts at %d ",ReadPtr+ATR.soff), "Accessed ", fnattr.atime.convertToIsoTime(),"Created ",fnattr.crtime.convertToIsoTime(),
			                    "Modified ",fnattr.mtime.convertToIsoTime(), "Filename ",fnattr.fname, fmt.Sprintf("MFT Entry Par ref %d Par. Seq Num. %d Type %s",fnattr.par_ref,fnattr.par_seq, Flags[fnattr.flags])}," ")
			...
			_,err:=file1.WriteString(s+string(10))
			    ...
			    r.Data=true
			     _,err:=file1.WriteString("Resident DATA ATTR"+string(10))
			    if err != nil {
				 // handle the error here
				 fmt.Printf("err %s\n",err)	
				   return
		      ...
		      // fmt.Println("file unique id ",objectattr.objid)
		       s := [] string  {"file unique id ",objectattr.objid,string(10)}
			 ...
			 s := [] string  {"Type of Attr in Run list", fmt.Sprintf("Attribute starts at %d",ReadPtr),
			   ...
			   s := [] string  {"Vol Name ",volname.name.PrintNulls(),string(10)}
			  _,err:=file1.WriteString(strings.Join(s," "))
			  if err != nil {
			    // handle the error here
			    fmt.Printf("err %s\n",err)	
			...
			// 
			  s := [] string  {idxroot.Type,"index record size",fmt.Sprintf("%d bytes ",idxroot.Sizebytes),"Index entry start",fmt.Sprintf("%d",16+idxroot.nodeheader.OffsetEntryList),
			        fmt.Sprintf(" used portion ends at %d",16+idxroot.nodeheader.OffsetEndUsedEntryList),fmt.Sprintf("allocated ends at %d",16+idxroot.nodeheader.OffsetEndEntryListBuffer)  ,
				 "contenxt of index",//fmt.Sprintf("%x",bs[uint32(ReadPtr)+uint32(ATR.soff)+32:uint32(ReadPtr)+uint32(ATR.soff)+16+idxroot.nodeheader.OffsetEndEntryListBuffer]
				  string(10)}
			  _,err:=file1.WriteString(strings.Join(s," "))
			  if err != nil {
			    // handle the error here
			...
			siattr:=SIAttribute{  WindowsTime{readEndian(bs[ReadPtr+ATR.soff:ReadPtr+ATR.soff+8]).(uint64)},
		       ...
		       s := [] string {fmt.Sprintf(" starts at %d",ReadPtr),"Standard info ->Created ",siattr.crtime.convertToIsoTime(),
		             "Accessed ",siattr.atime.convertToIsoTime(),"Modified ",siattr.mtime.convertToIsoTime(),"MFT modified time ",siattr.MFTmtime.convertToIsoTime(),string(10)}
			 _,err:=file1.WriteString(strings.Join(s," "))
		       if err != nil {
			    // handle the error here
			    fmt.Printf("err %s\n",err)	
		  ...
		  if ATR.len > 0   {
			...
			//  buffer:=[] byte{}// a slice
		          ...
		          s := [] string {"NON Resident type ", AttrTypes[ATR.Type],fmt.Sprintf("LEN %d",ATR.len),fmt.Sprintf("Start VCN %d END VCN %d",ATR.start_vcn,ATR.last_vcn ), string(10)}
			 _,err:=file1.WriteString(strings.Join(s," "))
		       if err != nil {
			    // handle the error here
			    fmt.Printf("err %s\n",err)	
			  ...
			  if uint32(ReadPtr)+ATR.len<=1024{
				...
				s := [] string {  fmt.Sprintf(" offset starts at %d goes for  %d clusters\n",ClustersOff,ClustersLen),
				     string(10)}
				 _,err:=file1.WriteString(strings.Join(s," "))
				if err != nil {
				  // handle the error here
